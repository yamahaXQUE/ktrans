...
ALTER TABLE task_candidates
    ADD COLUMN complaint_basis varchar(40) NOT NULL DEFAULT 'legacy',
    ADD COLUMN complaint_evidence text NOT NULL DEFAULT '';

UPDATE task_candidates
SET complaint_basis = 'none'
WHERE NOT should_create;

ALTER TABLE task_candidates
    ADD CONSTRAINT task_candidates_complaint_basis_check CHECK (
        complaint_basis IN (
            'legacy',
            'explicit_complaint',
    ...
    ADD CONSTRAINT task_candidates_complaint_gate_check CHECK (
        (
            NOT should_create
            AND task_type = 'none'
            AND complaint_basis = 'none'
            AND btrim(complaint_evidence) = ''
        )
        OR (
            should_create
            AND complaint_basis = 'legacy'
        )
        OR (
            should_create
            AND complaint_basis IN (
                'explicit_complaint',
                'explicit_negative_feedback'
            )
            AND btrim(complaint_evidence) <> ''
